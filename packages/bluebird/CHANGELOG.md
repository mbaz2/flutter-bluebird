...
- Fixed a scan cancelled while `startScan` was still in flight hanging forever — and the cancel with it. The generator reached its `yield*` already cancelled, so the advertisement controller was never listened to, and the cleanup then awaited a `close()` that can never complete on such a controller: `stopScan` was never sent, `isScanning` stayed `true`, and every later scan failed with `operationInProgress` for the life of the process. A consumer awaiting the cancel — as a refresh that stops the previous scan first does — waited forever too.
- (Darwin) `connect` now gives up at its `timeout` natively as well, instead of leaving the attempt outstanding until something cancels it.
